...
- **骑行界面**：记录 tab 落地就是它，没有单独的准备页。**整页是实时地图**，读数与按钮以半透明浮层压在上面——顶部四格「距离 · 时长 · 海拔 · 爬升」，底部 120pt 大速度 + 刻度尺 + 三格「心率 · 踏频 · 功率」。底部动作区按动作频率分宽——未开始是整宽「开始」，记录中是「结束」占 1/3 靠左、「暂停 / 继续」占 2/3 靠右。亮屏常亮，按电源键熄屏后采集与语音提示照常。没连上的心率 / 踏频 / 功率可点那一格就地重连。
- **骑行中的提示**：每满 1km 报该公里的均速与耗时，每满 10km 报累计里程，速度向上跨过 10km/h 的整档报当前速度。屏内横幅 + 中文语音播报同时走，熄屏放口袋里也听得到。
